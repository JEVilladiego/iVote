...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cast_vote') {
        ...
        // Verify candidate belongs to position + election (join through positions to avoid
        // relying on a direct election_id column on the candidates table).
        $chk = $db->prepare(
        ...
        // Selection is valid — acknowledge to client so it can update local state.
        // Nothing is written to the database here; votes are only persisted on finalize.
        echo json_encode(['status'=>'success']);
        exit;
    }

    // Finalize ballot — persist all votes atomically, then mark has_voted.
    // This is the ONLY place votes are written to the database.
    if ($action === 'finalize') {
        ...
        // The client submits the full votedMap as JSON: { positionId: candidateId, ... }
        $votedMapRaw = json_decode($_POST['voted_map'] ?? '{}', true);
        if (!is_array($votedMapRaw)) $votedMapRaw = [];

        // Sanitise: keep only valid integer keys/values and ignore empty slots
        $votedMapClean = [];
        foreach ($votedMapRaw as $posId => $candId) {
            // Skip any null or 0 values generated by accidental arrays
        ...
        // Server-side re-validation: verify every submitted candidate actually belongs
        // to the claimed position and election (JOIN through positions — candidates may
        // not have a direct election_id column), and that the student is eligible.
        ...
        // All checks passed — write votes and mark ballot complete atomically.
        try {
            $db->beginTransaction();
