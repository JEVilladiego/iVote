...
// Already fully voted? Use has_voted flag as the source of truth —
// it covers both voted and abstained positions (set on finalize).
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cast_vote') {
        ...
        // Verify candidate belongs to position + election
        $chk = $db->prepare("SELECT id, position_id FROM candidates WHERE id=? AND position_id=? AND election_id=?");
        ...
        // Guard: if this is a rep position, the student must belong to that program
        ...
        try {
            ...
            )->execute([$election['id'], $posId, $candId, $user['id']]);
            echo json_encode(['status'=>'success']);
        } catch (\PDOException $e) {
            echo json_encode(['status'=>'error','message'=>'Already voted for this position.']);
        }
        exit;
    }

    // Finalize ballot (mark has_voted, verify completeness)
    if ($action === 'finalize') {
        // Recompute the filtered position count server-side for this student,
        // so the completeness check is consistent with what was shown on the ballot.
        ...
        $voted  = $db->prepare("SELECT COUNT(*) FROM votes WHERE voter_id=? AND election_id=?");
        $voted->execute([$user['id'], $election['id']]);
        $votedN    = (int)$voted->fetchColumn();

        // Abstained positions are tracked client-side only (not rows in the votes table).
        // Cap abstained_count so it cannot exceed positions not already in votes,
        // preventing a spoofed value from bypassing the completeness check.
...
// Track state
const votedMap    = <?= json_encode($votedMapPHP) ?>;  // posId → candId
